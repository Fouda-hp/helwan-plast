...
class AccountantForm(AccountantFormTemplate):
    ...
    def _safe_call(self, fn_name, *args, **kwargs):
        """Wrap anvil.server.call with error handling so JS always gets a structured response."""
        try:
            return anvil.server.call(fn_name, *args, **kwargs)
        except Exception as e:
            logger.error("Server call '%s' failed: %s", fn_name, e)
            return {'success': False, 'message': str(e)}

    # --- Financial Reports ---
    def get_trial_balance(self, date_from='', date_to=''):
        return self._safe_call('get_trial_balance', date_from, date_to, self._auth())

    def get_income_statement(self, date_from='', date_to=''):
        return self._safe_call('get_income_statement', date_from, date_to, self._auth())

    def get_balance_sheet(self, as_of_date=''):
        return self._safe_call('get_balance_sheet', as_of_date, self._auth())
    ...
    def get_contract_profitability(self, contract_number=None):
        ...
        return self._safe_call('get_contract_profitability', contract_number, self._auth())

    # --- Payable Tracking ---
    def get_contract_payable_status(self, contract_number=None):
        """Per-contract: total, paid, remaining, payment history, import costs."""
        return self._safe_call('get_contract_payable_status', contract_number, self._auth())

    # --- Accounts ---
    def get_bank_accounts(self):
        """Cash + all bank sub-accounts for dropdowns."""
        return self._safe_call('get_bank_accounts', self._auth())

    def get_chart_of_accounts(self):
        return self._safe_call('get_chart_of_accounts', self._auth())

    def get_account_balance(self, account_code, as_of_date=''):
        return self._safe_call('get_account_balance', account_code, as_of_date, self._auth())

    # --- Expenses ---
    def get_expenses(self, date_from='', date_to='', category=''):
        return self._safe_call('get_expenses', date_from, date_to, category, self._auth())

    def add_expense(self, data):
        return self._safe_call('add_expense', data, self._auth())

    def delete_expense(self, expense_id):
        return self._safe_call('delete_expense', expense_id, self._auth())
    ...
    def seed_accounts(self):
        ...
        return self._safe_call('seed_accounts', self._auth())

    def migrate_import_costs(self):
        """ONE-TIME: reclassify old import costs from 5100 to 1200 Inventory."""
        return self._safe_call('migrate_import_costs_to_inventory', self._auth())
    ...
    def migrate_old_contracts(self, supplier_id, currency='USD', dry_run=False):
        ...
        return self._safe_call('migrate_old_contracts', supplier_id, currency, dry_run, self._auth())

    # --- Currency ---
    def get_exchange_rates(self):
        return self._safe_call('get_exchange_rates', self._auth())

    def set_exchange_rate(self, currency_code, rate_to_egp):
        return self._safe_call('set_exchange_rate', currency_code, rate_to_egp, self._auth())

    def delete_exchange_rate(self, currency_code):
        return self._safe_call('delete_exchange_rate', currency_code, self._auth())

    # --- Treasury & Opening Balances ---
    def get_treasury_summary(self):
        return self._safe_call('get_treasury_summary', self._auth())

    def get_opening_balances(self, entity_type=''):
        return self._safe_call('get_opening_balances', entity_type, self._auth())

    def set_opening_balance(self, name, entity_type, amount):
        return self._safe_call('set_opening_balance', name, entity_type, amount, self._auth())
    ...
    def post_opening_balances(self, financial_year):
        ...
        return self._safe_call('post_opening_balances', financial_year, self._auth())
    ...
    def get_cash_flow_report(self, date_from, date_to):
        return self._safe_call('get_cash_flow_report', date_from, date_to, self._auth())

    def generate_report(self, report_name, filters):
        return self._safe_call('generate_report', report_name, filters, self._auth())

    def export_report(self, report_name, filters, format='csv'):
        return self._safe_call('export_report', report_name, filters, format, self._auth())

    def get_period_locks(self, year=None):
        return self._safe_call('get_period_locks', year, self._auth())

    def close_period(self, year, month):
        return self._safe_call('close_period', year, month, self._auth())

    def reopen_period(self, year, month):
        return self._safe_call('reopen_period', year, month, self._auth())

    def close_financial_year(self, year, confirm=False):
        return self._safe_call('close_financial_year', year, self._auth(), confirm)
    ...
    def get_customer_summary(self):
        return self._safe_call('get_customer_summary', self._auth())

    def get_supplier_summary(self):
        return self._safe_call('get_supplier_summary', self._auth())

    def delete_opening_balance(self, name, entity_type):
        return self._safe_call('delete_opening_balance', name, entity_type, self._auth())
