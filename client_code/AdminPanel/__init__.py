...
class AdminPanel(AdminPanelTemplate):
        ...
        try:
            ...
            if token:
                ...
                if result.get('valid'):
                    ...
                    self.user_email = (self.current_user.get('email', '') or '').strip()
                    full_name = (self.current_user.get('full_name', '') or '').strip()
                    self.user_name = full_name or (self.user_email.split('@')[0] if '@' in self.user_email else '')
    ...
    def register_passkey(self, token=None):
        ...
        try:
            ...
            device_info = {}
            try:
                device_info = json.loads(anvil.js.window.eval("""
                    (function(){
    ...
    def list_user_passkeys(self, user_email):
        """Admin: List passkeys for any user by email (via server)."""
        try:
            return anvil.server.call('webauthn_admin_list_credentials', self.get_token(), user_email)
        except Exception as e:
            return {'success': False, 'error': str(e), 'credentials': []}

    def remove_user_passkey(self, user_email, credential_id_prefix):
        """Admin: Remove a passkey for any user (via server)."""
        try:
            return anvil.server.call('webauthn_admin_remove_credential', self.get_token(), user_email, credential_id_prefix)
        except Exception as e:
            return {'success': False, 'error': str(e)}
    ...
    def get_user_name(self):
        ...
        if self.user_name and self.user_name.lower() != 'admin':
        ...
        try:
            ...
            email = (self.user_email or anvil.js.window.sessionStorage.getItem('user_email') or '').strip()
            if email and '@' in email:
                pretty = email.split('@')[0]
                try:
                    anvil.js.window.sessionStorage.setItem('user_name', pretty)
                except Exception:
                    pass
                return pretty
        except Exception:
