...
class ContractPrintForm(ContractPrintFormTemplate):
    ...
    def update_payment_rows(self):
        ...
        for i in range(1, num + 1):
            label = labels.get(i, ('الدفعة ' + str(i), 'Installment ' + str(i)))
    ...
    def validate_payments(self):
        ...
        for i, (val_inp, date_inp) in enumerate(zip(value_inputs, date_inputs)):
            ...
            if val > 0 and not date_str:
                errors.append(
                    ('تاريخ الدفعة رقم ' + str(i+1) + ' ناقص') if is_ar else ('Date for installment ' + str(i+1) + ' is missing')
                )
            # دفعة ناقصة: لو فيه تاريخ ومفيش قيمة
            if date_str and val <= 0:
                errors.append(
                    ('قيمة الدفعة رقم ' + str(i+1) + ' ناقصة') if is_ar else ('Value for installment ' + str(i+1) + ' is missing')
                )
            ...
            if date_str:
                try:
                    ...
                    errors.append(
                        ('تاريخ غير صحيح للدفعة رقم ' + str(i+1)) if is_ar else ('Invalid date for installment ' + str(i+1))
                    )
                else:
                    if payment_date < today:
                        errors.append(
                            ('تاريخ الدفعة رقم ' + str(i+1) + ' لا يمكن أن يكون قبل اليوم') if is_ar else ('Date for installment ' + str(i+1) + ' cannot be before today')
                        )
                    ...
                    if last_date is not None and payment_date < last_date:
                        errors.append(
                            ('تاريخ الدفعة رقم ' + str(i+1) + ' لا يمكن أن يكون قبل تاريخ الدفعة السابقة') if is_ar else ('Date for installment ' + str(i+1) + ' cannot be before previous installment')
                        )
        ...
        if self.payment_method == 'percentage':
            if round(total_value, 2) != 100:
                errors.append(
                    ('إجمالي النسب = ' + str(total_value) + '% â€" يجب أن يكون 100%') if is_ar else ('Total = ' + str(total_value) + '% â€" must be 100%')
                )
        else:
            if round(total_value, 0) != round(total_price, 0):
                diff = abs(total_price - total_value)
                errors.append(
                    ('إجمالي المبالغ (' + "{:,.0f}".format(total_value) + ') لا يساوي قيمة العقد (' + "{:,.0f}".format(total_price) + ') â€" الفرق ' + "{:,.0f}".format(diff)) if is_ar else ('Total (' + "{:,.0f}".format(total_value) + ') does not match contract (' + "{:,.0f}".format(total_price) + ') â€" diff ' + "{:,.0f}".format(diff))
                )
            ...
            for i, (val_inp, date_inp) in enumerate(zip(value_inputs, date_inputs)):
                ...
                if val > 0:
                    ...
                    self.payment_data.append({
                        'index': i + 1,
                        'label_ar': labels_ar[i] if i < len(labels_ar) else ('الدفعة ' + str(i+1)),
        ...
        try:
            ...
            if result and result.get('success'):
                ...
                if result.get('purchase_invoice_number'):
                    ...
                    acct_msg = (' | فاتورة شراء: ' + str(inv_num)) if is_ar else (' | Purchase Invoice: ' + str(inv_num))
            ...
            is_ar = self.current_lang == 'ar'
            if is_ar:
                msg = 'المشكلة: فشل حفظ العقد. تفاصيل: ' + str(detail)
            else:
    ...
    def render_template(self):
        ...
        machine_type_display = ('Flexo Stack With ' + str(machine_type_base)) if not is_ar else ('فليكسو ستاك مع ' + str(machine_type_base))
        ...
        html += '<div class="section-title">' + ('جدول المواصفات الفنية:' if is_ar else 'General Specifications:') + '</div>'
        ...
        if colors_count == 8:
            colors_display = "8+0, 7+1, 6+2, 5+3, 4+4 reverse printing" if not is_ar else (str(to_ar('8+0')) + '، ' + str(to_ar('7+1')) + '، ' + str(to_ar('6+2')) + '، ' + str(to_ar('5+3')) + '، ' + str(to_ar('4+4')) + ' طباعة عكسية')
        elif colors_count == 6:
            colors_display = "6+0, 5+1, 4+2, 3+3 reverse printing" if not is_ar else (str(to_ar('6+0')) + '، ' + str(to_ar('5+1')) + '، ' + str(to_ar('4+2')) + '، ' + str(to_ar('3+3')) + ' طباعة عكسية')
        elif colors_count == 4:
            colors_display = "4+0, 3+1, 2+2 reverse printing" if not is_ar else (str(to_ar('4+0')) + '، ' + str(to_ar('3+1')) + '، ' + str(to_ar('2+2')) + ' طباعة عكسية')
        else:
        ...
        def ar_brake_power(bp_str):
            ...
            for part in parts:
                ...
                if m:
                    ar_parts.append(str(to_ar(m.group(1))) + ' قطعة (' + str(to_ar(m.group(2))) + ' كجم)')
                else:
        ...
        def ar_dryer(dc_str):
            ...
            if m:
                return str(to_ar(m.group(1))) + ' كيلو وات تجفيف هوائي × ' + str(to_ar(m.group(2)))
            return to_ar(dc_str)
        ...
        def ar_print_length(pl_str):
            ...
            if m:
                return str(to_ar(m.group(1))) + ' مم - ' + str(to_ar(m.group(2))) + ' مم'
            return to_ar(pl_str)
        ...
        if is_ar:
            value_map = {
                ...
                'tension_units': str(to_ar(tension_units)) + ' قطعة',
                'brake_system': str(to_ar(brake_system)) + ' قطعة',
                'brake_power': ar_brake_power(brake_power),
                'web_guiding': str(to_ar(web_guiding)) + ' قطعة',
                'max_film_width': str(to_ar(max_film_width)) + ' مم',
                'max_print_width': str(to_ar(max_print_width)) + ' مم',
                'print_length': ar_print_length(print_length),
                'max_roll_diameter': str(to_ar(max_roll_diameter)) + ' مم',
                'anilox_display': anilox_display,
                'max_machine_speed': str(to_ar(max_machine_speed)) + ' متر في الدقيقة',
                'max_print_speed': str(to_ar(max_print_speed)) + ' متر في الدقيقة',
                'dryer_capacity': ar_dryer(dryer_capacity),
                'drive_display': drive_display,
                'main_motor_power': str(to_ar(main_motor_power.replace('HP', '').replace('hp', '').strip())) + ' حصان',
            }
        else:
            value_map = {
                'model': data.get('model', '-'),
        ...
        html += '<div class="section-title">' + ('سلندرات الطباعة :' if is_ar else 'Printing Cylinders:') + '</div>'
        html += '<div style="display: flex; justify-content: center;">'
        html += '<table class="cylinders-table" style="width: 50%; margin: 0 auto;">'
        html += '<tr><th style="background:#f5f5f5; padding:8px; border:1px solid #ddd;">' + ('مقاس' if is_ar else 'Size') + '</th><th style="background:#f5f5f5; padding:8px; border:1px solid #ddd;">' + ('عدد' if is_ar else 'Count') + '</th></tr>'
        for i in range(12):
            if i < len(cylinders):
                cyl = cylinders[i]
                size = cyl.get("size", "")
        ...
        if self.payment_data:
            html += '<div class="section-title">' + ('جدول الدفعات:' if is_ar else 'Payment Schedule:') + '</div>'
            ...
            html += '<th style="padding:10px;color:white;">' + ('البند' if is_ar else 'Description') + '</th>'
            html += '<th style="padding:10px;color:white;">' + ('النسبة' if is_ar else '%') + '</th>'
        ...
        html += '<h4>' + ('التسليم :' if is_ar else 'Delivery:') + '</h4>'
        html += '<p>' + ('مكان التسليم :' if is_ar else 'Delivery location:') + ' <span class="highlight">' + str(data.get("delivery_location", "-")) + '</span></p>'
        delivery_time = self.delivery_date if self.delivery_date else data.get("expected_delivery_formatted", "-")
        html += '<p>' + ('تاريخ التسليم المتوقع :' if is_ar else 'Expected delivery:') + ' <span class="highlight">' + str(delivery_time) + '</span></p>'
        html += '</div>'
