...
class QuotationPrintForm(QuotationPrintFormTemplate):
  ...
  def render_template(self):
    ...
    if colors_count == 8:
      colors_display = "8+0, 7+1, 6+2, 5+3, 4+4 reverse printing" if not is_ar else (to_ar('8+0') + "، " + to_ar('7+1') + "، " + to_ar('6+2') + "، " + to_ar('5+3') + "، " + to_ar('4+4') + " طباعة عكسية")
    elif colors_count == 6:
      colors_display = "6+0, 5+1, 4+2, 3+3 reverse printing" if not is_ar else (to_ar('6+0') + "، " + to_ar('5+1') + "، " + to_ar('4+2') + "، " + to_ar('3+3') + " طباعة عكسية")
    elif colors_count == 4:
      colors_display = "4+0, 3+1, 2+2 reverse printing" if not is_ar else (to_ar('4+0') + "، " + to_ar('3+1') + "، " + to_ar('2+2') + " طباعة عكسية")
    else:
    ...
    def ar_brake_power(bp_str):
      ...
      for part in parts:
        ...
        if m:
          ar_parts.append(to_ar(m.group(1)) + " قطعة (" + to_ar(m.group(2)) + " كجم)")
        else:
    ...
    def ar_dryer(dc_str):
      ...
      if m:
        return to_ar(m.group(1)) + " كيلو وات تجفيف هوائي × " + to_ar(m.group(2))
      return to_ar(dc_str)
    ...
    def ar_print_length(pl_str):
      ...
      if m:
        return to_ar(m.group(1)) + " مم - " + to_ar(m.group(2)) + " مم"
      return to_ar(pl_str)
    ...
    if is_ar:
      value_map = {
        ...
        'tension_units': to_ar(tension_units) + " قطعة",
        'brake_system': to_ar(brake_system) + " قطعة",
        'brake_power': ar_brake_power(brake_power),
        'web_guiding': to_ar(web_guiding) + " قطعة",
        'max_film_width': to_ar(max_film_width) + " مم",
        'max_print_width': to_ar(max_print_width) + " مم",
        'print_length': ar_print_length(print_length),
        'max_roll_diameter': to_ar(max_roll_diameter) + " مم",
        'anilox_display': anilox_display,
        'max_machine_speed': to_ar(max_machine_speed) + " متر في الدقيقة",
        'max_print_speed': to_ar(max_print_speed) + " متر في الدقيقة",
        'dryer_capacity': ar_dryer(dryer_capacity),
        'drive_display': drive_display,
        'main_motor_power': to_ar(main_motor_power.replace('HP', '').replace('hp', '').strip()) + " حصان",
      }
    else:
      value_map = {
        'model': data.get('model', '-'),
    ...
    html += '<tr><th>' + ("مقاس" if is_ar else "Size") + '</th><th>' + ("عدد" if is_ar else "Count") + '</th></tr>'
    for i in range(12):
      if i < len(cylinders):
        cyl = cylinders[i]
        size = cyl.get("size", "")
    ...
    html += '<p>' + ("مكان التسليم :" if is_ar else "Place of delivery:") + ' <span class="highlight">' + str(data.get("delivery_location", "-")) + '</span></p>'
    if is_in_stock:
    ...
    html += '<p>' + ("وقت التسليم المتوقع :" if is_ar else "Expected delivery time:") + ' <span class="highlight">' + str(delivery_time) + '</span></p>'
    html += '</div>'
    ...
    html += '<div class="regards">' + ("وتفضلوا بقبول وافر الاحترام،،،" if is_ar else "Yours faithfully,") + '</div>'
