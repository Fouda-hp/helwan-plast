...
def build_pdf_data(q_data, user_info, sales_rep_info, company_settings, machine_specs, tech_specs_settings):
    ...
    total_price = _safe_float(q_data.get('Agreed Price'), 0)
    down_percent = _safe_float(company_settings.get('down_payment_percent'), 0)
    shipping_percent = _safe_float(company_settings.get('before_shipping_percent'), 0)
    delivery_percent = _safe_float(company_settings.get('before_delivery_percent'), 0)
