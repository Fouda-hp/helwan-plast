  ...
  if (!window.showConfirm) {
    window.showConfirm = function(msg, title) {
      return new Promise(function(resolve) {
        ...
        b.innerHTML = '<div style="background:#fff;padding:24px;border-radius:12px;max-width:400px;box-shadow:0 8px 32px rgba(0,0,0,0.2);"><div style="font-weight:600;margin-bottom:12px;">' + _esc(title || '\u062a\u0623\u0643\u064a\u062f') + '</div><div style="margin-bottom:20px;">' + _esc(msg || '') + '</div><div style="display:flex;gap:10px;justify-content:flex-end;"><button id="hpCnNo" style="padding:10px 20px;border:1px solid #ccc;border-radius:8px;background:#fff;cursor:pointer;">\u0644\u0627</button><button id="hpCnYes" style="padding:10px 20px;border:none;border-radius:8px;background:#1976d2;color:#fff;cursor:pointer;">\u0646\u0639\u0645</button></div></div>';
  ...
  if (!window.showPrompt) {
    window.showPrompt = function(msg, def, title) {
      return new Promise(function(resolve) {
        ...
        b.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999999;display:flex;align-items:center;justify-content:center;';
        b.innerHTML = '<div style="background:#fff;padding:24px;border-radius:12px;max-width:400px;box-shadow:0 8px 32px rgba(0,0,0,0.2);">' + (title ? '<div style="font-weight:600;margin-bottom:12px;">' + _esc(title) + '</div>' : '') + '<div style="margin-bottom:12px;">' + _esc(msg || '') + '</div><input type="text" id="' + id + '" value="' + (def != null ? _esc(String(def)) : '') + '" style="width:100%;padding:10px;border:1px solid #ccc;border-radius:8px;margin-bottom:16px;box-sizing:border-box;"><div style="display:flex;gap:10px;justify-content:flex-end;"><button id="hpPmCancel" style="padding:10px 20px;border:1px solid #ccc;border-radius:8px;background:#fff;cursor:pointer;">\u0625\u0644\u063a\u0627\u0621</button><button id="hpPmOk" style="padding:10px 20px;border:none;border-radius:8px;background:#1976d2;color:#fff;cursor:pointer;">\u0645\u0648\u0627\u0641\u0642</button></div></div>';
